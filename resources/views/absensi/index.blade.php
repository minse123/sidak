        ...
        <div class="bg-white border-0 shadow-sm rounded-4">
            <div
                ...
                <div>
                    ...
                    <h1 class="h2 fw-bold text-dark mb-2">Kelola data Penerimaan Negara Bukan Pajak</h1>
                    <p class="text-muted mb-0">
                        Pantau dan verifikasi setiap transaksi PNBP yang masuk.
                    </p>
                </div>
                    ...
                    <table class="table table-hover align-middle mb-0">
                        <thead class="table-light">
                            <tr class="text-center">
                                <th scope="col" class="px-4 text-center" style="width: 60px;">#</th>
                                ...
                                <th scope="col" class="text-center pe-4" style="width: 180px;">Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse ($pnbps as $pnbp)
                                <tr>
                                    <td class="px-4 fw-semibold">
                                        {{ ($pnbps->currentPage() - 1) * $pnbps->perPage() + $loop->iteration }}</td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="text-dark">Rp {{ number_format($pnbp->total_potongan, 0, ',', '.') }}
                                        </div>
                                        ...
                                        <div class="d-flex justify-content-end gap-1">
                                            @can('verify', $pnbp)
                                                @if ($pnbp->status === 'Pending')
                                                    <form action="{{ route('pnbp.verify', $pnbp) }}" method="POST"
                                                        onsubmit="return confirm('Verifikasi data PNBP ini?');"
                                                        class="d-inline">
                                                        @csrf
                                                        @method('PATCH')
                                                        <button type="submit"
                                            ...
                                            @can('delete', $pnbp)
                                                <form action="{{ route('pnbp.destroy', $pnbp) }}" method="POST"
                                                    onsubmit="return confirm('Hapus data PNBP ini?');" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="text-center py-5">
                                        ...
                                        <a href="{{ route('pnbp.create') }}" class="btn btn-link">Tambahkan data PNBP
                                            pertama</a>
                                    </td>
                                </tr>
                            @endforelse
